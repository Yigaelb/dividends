...
###
# This script updates a CSV file with dividend information extracted from a markdown file.
# It handles duplicate entries, validates dates, and normalizes company names.
# It also provides options for automatic file detection and manual specification.
# The script can be run with command line arguments to specify input and output files.
###S
...
def parse_md_file(md_file_path):
    ...
    # Pattern to match company name and acronym from markdown link
    pattern = r'\|\|\*\*([^*]+?)\*\*\(\[([^\]]+?)\]\([^)]+?\)\)\|(\d{2}\.\d{2}\.\d{4})\|(\d+\.?\d*)\|\|(\d{2}\.\d{2}\.\d{4})\|'
    
    # Process the content and remove duplicates within the MD file
    content_lines = content.split('\n')
    data_lines = []
    seen_entries = set()  # Track unique entries by company, date, and amount
    
    for line in content_lines:
        # Skip header lines and date headers
        if ('--' in line or '``' in line or 'יום' in line or not line.strip()):
            continue
        # Remove all spaces
        ...
        # Try to extract the entry details
        match = re.search(pattern, line)
        if match:
            company, acronym, ex_date, dividend, payment_date = match.groups()
            entry_key = f"{company.strip()} ({acronym})_{ex_date}_{dividend}"
            if entry_key not in seen_entries:
                seen_entries.add(entry_key)
                data_lines.append(line)
            else:
                print(f"Skipping duplicate in MD file: {company.strip()} ({acronym}) on {ex_date}")
    
    content_filtered = '\n'.join(data_lines)
    print(f"\nProcessing file: {md_file_path}")
    
    entries = []
    matches = list(re.finditer(pattern, content_filtered))
    print(f"Found {len(matches)} entries in file")
    
    for match in matches:
        try:
            company, acronym, ex_date, dividend, payment_date = match.groups()
            company_name = company.strip()
            company_with_acronym = f"{company_name} ({acronym})"  # Combine name and acronym
            ...
            entries.append({
                ...
                'תשואה': '',  # Empty string for 'תשואה' column
    ...
    return pd.DataFrame(entries), len(matches)  # Return both the DataFrame and total matches found
...
USAGE:
    python dividend_updater_2025_05_27.py [OPTIONS]
...
EXAMPLES:
    python dividend_updater_2025_05_27.py
        (Use automatic file detection)
    
    python dividend_updater_2025_05_27.py --list
        (List available files)
    
    python dividend_updater_2025_05_27.py --md "update_jan.md" --csv "dividends.csv"
...
def main():
    ...
    csv_file = args.csv or find_latest_file("*.csv", "Dividend") or find_latest_file("*Historical_Dividend*.csv", None)
